...
class MapElitesRepertoire(flax.struct.PyTreeNode):
    ...
    def sample(self, random_key: RNGKey, num_samples: int) -> Tuple[Genotype, RNGKey]:
        ...
        return_samples = jax.tree_util.tree_map(
            lambda x: jax.random.choice(subkey, x, shape=(num_samples,), p=p),
            self.returns,
        )
        ...
        return samples, return_samples, random_key, trajectory_samples
    ...
    def add(
        ...
        new_repertoire_returns = jax.tree_util.tree_map(
            lambda repertoire_returns, new_returns: repertoire_returns.at[
                batch_of_indices.squeeze(axis=-1)
            ].set(new_returns),
            self.returns,
            batch_of_extra_scores["returns"],
        )
        
        new_repertoire_trajectories = jax.tree_util.tree_map(
            lambda repertoire_trajectories, new_trajectories: repertoire_trajectories.at[
                batch_of_indices.squeeze(axis=-1)
            ].set(new_trajectories),
            self.trajectories,
            batch_of_extra_scores["transitions"],
        )
        ...
        return (
            ...
            returns=new_repertoire_returns,
            trajectories=new_repertoire_trajectories,
        ),
    ...
    def init(
        ...
        first_return = jax.tree_util.tree_map(lambda x: x[0], extra_scores["returns"])
        first_trajectory = jax.tree_util.tree_map(lambda x: x[0], extra_scores["transitions"])

        # create a repertoire with default values
        repertoire = cls.init_default(genotype=first_genotype, centroids=centroids, returns=first_return, trajectories=first_trajectory)
    ...
    def init_default(
        ...
        default_returns = jax.tree_util.tree_map(
            lambda x: jnp.zeros(shape=(num_centroids,) + x.shape, dtype=x.dtype),
            returns,
        )
        
        default_trajectories = jax.tree_util.tree_map(
            lambda x: jnp.zeros(shape=(num_centroids,) + x.shape, dtype=x.dtype),
            trajectories,
        )
