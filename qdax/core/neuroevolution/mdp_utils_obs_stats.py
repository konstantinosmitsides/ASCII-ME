...
def generate_unroll(
    ...
    (state, policy_params, _, obs_mean, obs_var), transitions = jax.lax.scan(
        _scan_play_step_fn,
        (init_state, policy_params, random_key, obs_mean, obs_var),  # Include initial 'value' as None if used
        None,
        length=episode_length
    )
