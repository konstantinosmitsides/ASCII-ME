...
def get_standardized_return(
    ...
    return_ = jax.vmap(get_return, in_axes=(0, None))(rewards * mask, episode_length)
    return standardize(return_)
    #return return_
...
def scoring_function_brax_envs_mcpg(
    ...
    standardized_returns= get_standardized_return(data.rewards, 1-mask, episode_length)
    data = data.replace(rewards=standardized_returns, dones=mask)
...
def reset_based_scoring_function_brax_envs_mcpg(
    policies_params: Genotype,
    random_key: RNGKey,
    episode_length: int,
    play_reset_fn: Callable[[RNGKey], EnvState],
    ...
    fitnesses, descriptors, extra_scores, random_key = scoring_function_brax_envs_mcpg(
        policies_params=policies_params,
        random_key=random_key,
        init_states=init_states,
        episode_length=episode_length,
