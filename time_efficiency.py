...
def plot(df):
    ...
    for col, env in enumerate(ENV_LIST):
        ...
        if env in ["walker2d_uni_1000", "ant_omni_250", "anttrap_omni_250"]:
            df_plot = df_plot[df_plot["algo"] != "pga_me"]
        ...
        num_bars = len(df_plot["algo"].unique())
        xlim_lower = -0.5
        xlim_upper = num_bars - 0.5
        ax.set_xlim(xlim_lower, xlim_upper)
        
        # Set uniform bar width and adjust positions
        bar_width = 0.8  # Adjust width as necessary
        for patch in ax.patches:
            patch.set_width(bar_width)
            current_position = patch.get_x()
            patch.set_x(current_position + (0.8 - bar_width) / 2)  # Adjust position based on width
    ...
    # Save the plot
    fig.savefig("data_time_efficiency/output/time_efficiency.pdf", bbox_inches="tight")
    plt.close(fig)
