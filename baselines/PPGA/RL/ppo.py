...
class PPO:
    ...
    def train(self, vec_env, num_updates, rollout_length, calculate_dqd_gradients=False, move_mean_agent=False,
        ...
        for update in range(1, num_updates + 1):
            with torch.no_grad():
                for step in range(rollout_length):
                    ...
                    self.next_obs, reward, dones, infos = vec_env.step(action)  # ENV_STEPS = (NO_OF_AGENTS) STEPS
                    if self.cfg.normalize_obs:
                        self.next_obs = self.vec_inference.vec_normalize_obs(self.next_obs)
            ...
            with torch.inference_mode():
                ...
                fps = self.global_step / train_elapse
                if not calculate_dqd_gradients and not move_mean_agent:  # backwards compatibility for standard PPO
                    if update % 10 == 0:
                        ...
                        log.debug(f'Avg FPS so far: {fps:.2f}, env steps: {self.global_step}, train time: {train_elapse:.2f}, avg reward: {np.mean(self.episodic_returns)}')
        ...
        log.debug(f'FPS: {fps:.2f}, Global Steps: {self.global_step}')
        if self.cfg.use_wandb:
            wandb.log({'FPS: ': fps})
    ...
    def evaluate(self, vec_agent, vec_env, verbose=True, obs_normalizer=None, return_normalizer=None):
        ...
        while not torch.all(dones): # and traj_length < num_steps:
            with torch.no_grad():
                ...
                obs, rew, next_dones, infos = vec_env.step(acts)    # ENV_STEPS = (NO_AGENTS) STEPS

                self.global_step += vec_env.num_envs
        ...
        print(f"num of envs: {vec_env.num_envs}")
